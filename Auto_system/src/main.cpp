...
{
    int16_t error = 0;
    lineDetected = readLineError(error);

    if (lineDetected)
    {
        currentLineError = error;
    }
}
...
void followLine()
{
    ...
    if (lineDetected)
    {
        ...
        const float derivative = dtSeconds > 0.0f
            ? (static_cast<float>(currentLineError) - lastLineError) / dtSeconds
            : 0.0f;

        const int16_t turn = clampTurn(
            LINE_TURN_DIRECTION
            * (currentLineError * LINE_TURN_KP + derivative * LINE_TURN_KD)
        );
